...
function App() {
  ...
  const handleWaveformMouseMove = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      ...
      if (!(e.buttons & 1)) { dragStart.current = null; return }
      const t = getTimeFromEvent(e)
      const [s, end] = dragStart.current < t
        ? [dragStart.current, t]
        : [t, dragStart.current]
      if (end - s > 0.05) {
  ...
  const selStartPct   = editor.selection && editor.duration > 0
    ? (editor.selection.startTime / editor.duration) * 100 : 0
  const selWidthPct   = editor.selection && editor.duration > 0
    ? ((editor.selection.endTime - editor.selection.startTime) / editor.duration) * 100 : 0
  const selDuration   = editor.selection
    ? editor.selection.endTime - editor.selection.startTime : 0
    ...
    <main className="app-shell">
      ...
      <header className="topbar">
        ...
        <div className="topbar-actions">
          ...
          <button className="avatar"      aria-label="Account">AC</button>
        </div>
      </header>
          ...
          <div className="editor-card">
            ...
            <div className="waveform-wrap">
              ...
              >
                ...
                <div className="playhead" style={{ left: `${positionPct}%` }} aria-hidden="true" />
                {bars.map((h, i) => (
                  <span
                    key={i}
                    className={(i / BAR_COUNT * 100) < positionPct ? 'played' : ''}
                    style={{ height: `${h}%` }}
                    aria-hidden="true"
                  />
                ))}
            ...
            <div className="tool-row">
              <div className="tool-group">
                ...
                <button className="edit-button" onClick={editor.applyTrim}   disabled={!editor.selection} title="Keep only the selected region">Trim</button>
                <button className="edit-button" onClick={editor.applyDelete} disabled={!editor.selection} title="Remove selected region">Delete</button>
                <button className="edit-button" onClick={editor.applySplit}  disabled={!editor.selection} title="Split at midpoint of selection">Split</button>
              </div>
              <div className="tool-group">
                <span className="tool-label">Effects</span>
                <button className="edit-button" onClick={editor.applyFadeIn}  disabled={!editor.selection}>Fade in</button>
                <button className="edit-button" onClick={editor.applyFadeOut} disabled={!editor.selection}>Fade out</button>
              </div>
            </div>
        ...
        {exportOpen && (
          <div className="export-panel">
            ...
            <div className="panel-actions">
              <button className="text-button" onClick={() => setExportOpen(false)}>Cancel</button>
              <button
                className="primary-button"
                onClick={() => {
                  setExportOpen(false)
