...
function replayOps(source: AudioBuffer, ops: EditOperation[]): AudioBuffer {
  ...
  for (const op of ops) {
    switch (op.type) {
      case "trim":
        buf = applyTrim(buf, op.params as TrimParams);
        break;
      case "cut":
        buf = applyCut(buf, op.params as CutParams);
        break;
      case "split":
        buf = applySplit(buf, op.params as SplitParams)[0];
        break;
      case "volume":
        buf = applyVolume(buf, op.params as VolumeParams);
        break;
      case "fade-in":
        buf = applyFadeIn(buf, op.params as FadeInParams);
        break;
      case "fade-out":
        buf = applyFadeOut(buf, op.params as FadeOutParams);
        break;
    }
  }
  return buf;
...
export function useEditor(): EditorState & EditorActions {
  ...
  const setSelection = useCallback(
    (r: SelectionRegion | null) => setSelectionState(r),
    [],
  );
  ...
  const requireSel = (sel: SelectionRegion | null): sel is SelectionRegion => {
    if (!sel) {
      setNotice("Select a region of the waveform first.");
      return false;
    }
    return true;
  };
  ...
  return {
    ...
    seek,
    setSelection,
    applyTrim: applyTrimAction,
    applyDelete: applyDeleteAction,
    applySplit: applySplitAction,
    applyFadeIn: applyFadeInAction,
    applyFadeOut: applyFadeOutAction,
    applyVolumeUp: applyVolumeUpAction,
